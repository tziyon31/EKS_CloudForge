...
def prometheus_metrics():
    ...
    # System metrics
    metrics.append(
        f"# HELP container_cpu_usage_seconds_total Total CPU usage in seconds"
    )
    metrics.append(f"# TYPE container_cpu_usage_seconds_total counter")
    metrics.append(
        f'container_cpu_usage_seconds_total{{container="eks-cloudforge-app"}} {cpu_percent / 100}'
    )
    ...
    metrics.append(
        f'container_memory_usage_bytes{{container="eks-cloudforge-app"}} {memory.used}'
    )
    ...
    metrics.append(
        f'container_fs_usage_bytes{{container="eks-cloudforge-app"}} {disk.used}'
    )
    ...
    metrics.append(
        f'container_fs_limit_bytes{{container="eks-cloudforge-app"}} {disk.total}'
    )
    ...
    metrics.append(
        f'http_requests_total{{job="eks-cloudforge-app",status="200"}} {REQUEST_COUNT}'
    )

    metrics.append(
        f"# HELP http_request_duration_seconds_sum Sum of HTTP request durations"
    )
    metrics.append(f"# TYPE http_request_duration_seconds_sum counter")
    metrics.append(
        f'http_request_duration_seconds_sum{{job="eks-cloudforge-app"}} {REQUEST_COUNT * 0.1}'
    )
    ...
    metrics.append(
        f'http_request_duration_seconds_count{{job="eks-cloudforge-app"}} {REQUEST_COUNT}'
    )
    ...
    metrics.append(
        f'app_cost_per_hour{{job="eks-cloudforge-app"}} {estimated_cost_per_hour}'
    )
